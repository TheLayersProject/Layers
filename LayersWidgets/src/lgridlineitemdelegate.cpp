#include <LayersWidgets/lgridlineitemdelegate.h>
...
void LGridlineItemDelegate::init_attributes()
{
	...
	connect(m_grid->fill(), &LAbstractAttribute::changed, [this] { emit changed(); });
	connect(m_grid->thickness(), &LAbstractAttribute::changed, [this] { emit changed(); });

...
LGradientDialog::LGradientDialog(QWidget* parent) :
...
{
    LColorControl* color_control = new LColorControl(this);
    color_control->show();
    color_control->fill()->set_value(color);

    color_control->disable_clicking();

    connect(color_control, &LColorControl::color_changed, [this, color_control] {
...
void LGradientDialog::set_gradient_stops(QGradientStops gradient_stops)
{
...
void LGradientDialog::init_attributes()
{
    for (LAbstractAttribute* entity :
        QList<LAbstractAttribute*>({
            m_border,
            m_border->thickness(),
            m_margins,
            m_margins->left(),
            ...
            m_margins->bottom()}))
    {
        connect(entity, &LAbstractAttribute::changed,
...
{
    for (LColorControl* color_control : m_color_controls)
...
void LGradientDialog::update_gradient()
{
...
void LGradientDialog::click_control()
{
...
void LGradientDialog::update_color_control_positions()
{
    if (!isVisible())
    ...
    {
        QGradientStop gradient_stop = m_gradient_stops.at(i);
        LColorControl* color_control = m_color_controls.at(i);
...
bool LGradientDialog::eventFilter(QObject* object, QEvent* event)
{

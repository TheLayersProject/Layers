#include <LayersWidgets/lwidgeteditor.h>
...
#include <LayersWidgets/ldialog.h>
	...
	m_themeable{ themeable }, LWidget(parent)
{
	...
	connect(m_show_all_button, &LButton::clicked, [this] {
		for (LAttributeEditor* aw : m_modifier_widgets)
			aw->show();
	...
	connect(m_show_enabled_button, &LButton::clicked, [this] {
		for (LAttributeEditor* aw : m_modifier_widgets)
			if (aw->disabled())
				aw->hide();
...
void LWidgetEditor::add_attribute_editor(LAttributeEditor* editor)
{
...
void LWidgetEditor::handle_editor(LAbstractAttribute* abs_attr, LAttributeEditor* editor)
{
	...
	connect(editor, &LAttributeEditor::widget_disabled, [this, editor] {
...
void LWidgetEditor::init_attribute_editors()
{
	...
	for (LAbstractAttribute* abs_attribute : m_themeable->attributes())
	{
		if (LAttribute* attribute = dynamic_cast<LAttribute*>(abs_attribute))
		{
			handle_editor(attribute, create_editor(attribute));
		}
		else if (LAttributeGroup* attr_group = dynamic_cast<LAttributeGroup*>(abs_attribute))
		{
			...
			{
				LAttributeEditorGroup* aw_group = new LAttributeEditorGroup(attr_group);
				aw_group->entangle_with(m_control_attribute_editor_group);

				for (LAttribute* attribute : attr_group->attributes())

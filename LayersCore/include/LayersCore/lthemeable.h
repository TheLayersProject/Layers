...
namespace Layers
{
	...
	/*!
		The LThemeable class is designed to be inherited alongside QWidget
		...
		variables in classes that implement %LThemeable. These subclasses will
		...
		by the LApplication, you will not need to call apply_theme()
		manually</b>.
		...
		the LThemeEditor. This causes copy_attribute_values_to() to be called on
	...
	*/
	class LAYERS_CORE_EXPORT LThemeable
	{
	public:
		~LThemeable();
		...
		*/
		QMap<QString, LAbstractAttribute*>& attributes();
		...
		*/
		...
		/*!
			...
			LAbstractAttribute::changed() signal is emitted by
			*abstract_attribute*.
		*/
		...
		*/
		...
		/*!
			...
			*disabled*.
			...
			active preview widget of the LThemeEditor.
		*/
	...
	protected:
		QMap<QString, LAbstractAttribute*> m_attributes;
	...
	template<typename T>
	inline void LThemeable::entangle_with(T* themeable, bool entangle_children)
	{
		if (typeid(*this) == typeid(*themeable))
		{
			for (const QString& entity_key : m_attributes.keys())
			{
				if (LAttribute* attr = dynamic_cast<LAttribute*>(m_attributes[entity_key]))
					attr->entangle_with(*dynamic_cast<LAttribute*>(themeable->m_attributes[entity_key]));
				else if (LAttributeGroup* attr_group = dynamic_cast<LAttributeGroup*>(m_attributes[entity_key]))
					attr_group->entangle_with(*dynamic_cast<LAttributeGroup*>(themeable->m_attributes[entity_key]));
			}

			if (entangle_children)
				for (LThemeable* this_child_themeable : child_themeables())
					if (this_child_themeable->m_name)
						for (LThemeable* child_themeable : themeable->child_themeables())

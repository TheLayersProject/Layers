...
void LScrollBar::init_attributes()
{
	...
	connect(m_background_color, &LAttribute::changed, [this] { update_stylesheet(); });
	...
	connect(m_handle_corner_radii->top_left(), &LAttribute::changed, [this] { update_stylesheet(); });
	connect(m_handle_corner_radii->top_right(), &LAttribute::changed, [this] { update_stylesheet(); });
	connect(m_handle_corner_radii->bottom_left(), &LAttribute::changed, [this] { update_stylesheet(); });
	connect(m_handle_corner_radii->bottom_right(), &LAttribute::changed, [this] { update_stylesheet(); });
	connect(m_handle_color, &LAttribute::changed, [this] { update_stylesheet(); });
}
